...
from definitions.predicates import list_of_timeranges_inside_booked_slot, get_booking_slot_distance_penalties, \
    get_booking_slot_distance_total_penalty
...
class TestGetBookingSlotDistanceTotalPenalty(TestCase):
    def setUp(self) -> None:
    ...
    def test__should_return_empty_list_for_empty_list(self):
        result = self.prolog.query(get_booking_slot_distance_total_penalty(
            [], Result
        ))
    ...
    def test__multiple_item(self):
        ...
        result = get(self, result, Result)
        self.assertEqual(sum([4, 0, 1, 3, 0, 2]), result)
